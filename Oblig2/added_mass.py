...
for i, geometry_data in enumerate(file_list):
    ff_22_sum =  calc_force_coefficents(os.path.join(data_dir_path, geometry_data))
    wave_number_array = np.arange(0.01, 2, 0.1)
    a22_coef = np.real(ff_22_sum)
    b22_coef = -1 * np.imag(ff_22_sum)

    axs[i//2, i%2].plot(wave_number_array, a22_coef, label = r'$\frac{a_{22}}{\rho D^{2}}$')
    axs[i//2, i%2].plot(wave_number_array, b22_coef, label = r'$\frac{b_{22}}{\rho D^{2}}$')
    axs[i//2, i%2].set_title('Added mass and damping coef. for geometry ' + aspect_ratio_strings[i])
    axs[i//2, i%2].set_xlabel(r'$\frac{\omega^{2}D}{g}$')
    axs[i//2, i%2].axvline(k_swell_wave, color='navy', alpha = 0.5, linestyle='--')
    mid_text_pos = max(a22_coef.max(), b22_coef.max())/2
    tx = axs[i//2, i%2].text(0.1, mid_text_pos, "wind waves")
